...
impl CPU {
    ...
    pub fn addr_zero_page<B: Bus>(&mut self, cycles: &mut u32, bus: &mut B) -> Byte {
        self.fetch_byte(cycles, bus)
    }
    /// ### Addressing Modes - Zero page with X offset
    pub fn addr_zero_page_x<B: Bus>(&mut self, cycles: &mut u32, bus: &mut B) -> Byte {
        let addr = self.fetch_byte(cycles, bus).wrapping_add(self.x);
        *cycles -= 1;
        addr
    }
    /// ### Addressing Modes - Zero page with Y offset
    pub fn addr_zero_page_y<B: Bus>(&mut self, cycles: &mut u32, bus: &mut B) -> Byte {
        let addr = self.fetch_byte(cycles, bus).wrapping_add(self.y);
        *cycles -= 1;
        addr
    }
    /// ### Addressing Modes - Absolute
    pub fn addr_absolute<B: Bus>(&mut self, cycles: &mut u32, bus: &mut B) -> Word {
    ...
    pub fn addr_indirect<B: Bus>(&mut self, cycles: &mut u32, bus: &mut B) -> Word {
        let addr = self.fetch_word(cycles, bus);
        self.read_word(cycles, addr, bus)
    }
    /// ### Addressing Modes - Indexed Indirect (X)
    pub fn addr_indirect_x<B: Bus>(&mut self, cycles: &mut u32, bus: &mut B) -> Word {
